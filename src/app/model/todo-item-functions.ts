import { convertSecondsToMS, isNullOrUndefined } from './date-time-seconds';
  ...
  (a: TodoItem, b: TodoItem): number => {
    ...
    switch (itemSort.field) {
      ...
      case 'createdDateTime':
        sortNumber =
          convertSecondsToMS(a.createdDateTime) -
          convertSecondsToMS(b.createdDateTime);
        break;
      default:
        sortNumber = 1;
    }
...
export function sortAndFilterData(
  ...
  return data.filter(filterTodoItemBy(filter)).sort(sortTodoItemBy(itemSort));
}
